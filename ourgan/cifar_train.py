...
import sys
import argparse
from cifar_network import Generator, Discriminator
from dataset import *
import pdb
# import tensorboardX
import os
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if (epoch + 1) % self.args.eval_interval == 0:
                ...
                torch.save(self.G, os.path.join(args.savedir, 'G.pkl'))
                torch.save(self.D, os.path.join(args.savedir, 'D.pkl'))
                save_image(gen_imgs.data[:25], 'images_1000/%d.png' % epoch, nrow=5)
                test_accuracy = float(self.eval()) / float(self.test.__len__())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', type=str, default='./models_cifar_fin', metavar='SAVE_PATH',
    ...
    gan = ImprovedGAN(Generator(100), Discriminator(), Cifarlabel(10), CifarUnlabel(), CifarTest(), args)
    gan.train()