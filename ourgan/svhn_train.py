...
import sys
import argparse
from svhn_network import Generator, Discriminator
from dataset import *
import pdb
# import tensorboardX
import os
...
class ImprovedGAN(object):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if (epoch + 1) % self.args.eval_interval == 0:
                print("Eval: correct %d / %d" % (self.eval(), 10000))
                torch.save(self.G, os.path.join(args.savedir, 'G.pkl'))
                torch.save(self.D, os.path.join(args.savedir, 'D.pkl'))
                save_image(gen_imgs.data[:25], 'images_svhn/%d.png' % epoch, nrow=5)
                test_accuracy = float(self.eval()) / float(10000)
        ...
        filename = 'test_accuracies'
        outfile = open(filename, 'wb')
        pickle.dump(test_accuracies, outfile)
        outfile.close()
    ...
    def eval(self):
        ...
        self.D.eval()
        d, l = [], []
        perm = np.random.permutation(self.test.__len__())
        cnt = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', type=str, default='./svhn_models', metavar='SAVE_PATH',
    ...
    gan = ImprovedGAN(Generator(100), Discriminator(), SVHNlabel(10), SVHNUnlabel(), SVHNTest(), args)
    gan.train()
